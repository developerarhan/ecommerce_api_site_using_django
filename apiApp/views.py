...
from .serializers import ProductListSerializer, ProductDetailSerializer, CategoryListSerializer, CategoryDetailSerializer, CartItemSerializer, CartSerializer, CartStatSerializer, ReviewSerializer, WishlistSerializer
...
def add_review(request):
    ...
    if Review.objects.filter(product=product, user=user).exists():
        return Response("You have already created a review for this product.")
...
def delete_review(request, pk): 
    review = Review.objects.get(id=pk)
    review.delete()
    return Response("Review have been deleted successfully!", status=204)
        ...
        checkout_session = stripe.checkout.Session.create(
            ...
            line_items=[{
                'price_data': {
                    ...
                    'unit_amount': int(item.product.price) * 100,  # amount in cents ($10.00)
                },
...
def my_webhook_view(request):
    ...
    if(
        ...
        print("Webhook triggered, calling fulfill_checkout...")
